...
static inline void transform(char *buffer, uint32_t len, const struct xt_xor_info *info)
{
    ...
    static unsigned int
xt_xor_target4(struct sk_buff *skb, const struct xt_action_param *par)
#else
    static unsigned int
xt_xor_target4(struct sk_buff *skb, const struct xt_target_param *par)
#endif
{
...
#if LINUX_VERSION_CODE >= KERNEL_VERSION(3,7,0)
    static unsigned int
xt_xor_target6(struct sk_buff *skb, const struct xt_action_param *par)
#else
    static unsigned int
xt_xor_target6(struct sk_buff *skb, const struct xt_target_param *par)
#endif
{
    const struct xt_xor_info *info = par->targinfo;
    struct ipv6hdr *ip6h;
